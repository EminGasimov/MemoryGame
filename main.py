...
def validate_user_input(player_choice, board):
    ...
    if col < 0 or col > NUM_COLS - 1:
        print("The col number must be between 0 and ", NUM_COLS - 1, ".", sep="")
        return False
...
def get_player_choices(board):
    ...
    while not valid_input:
        print("Enter the ROW and COLUMN number for your FIRST choice of card, ")
        player_choice = input("separated by a space: ")
    ...
    valid_input = False
    while not valid_input:
        print("Enter the ROW and COLUMN number for your SECOND choice of card, ")
        player_choice = input("separated by a space: ")
...
def main():  
  ...
  print(f'You completed games in {player_turns} turns')
